javascript: (() => { /* eslint-disable-line no-unused-labels */
  ...
  function AutoHighlighter() {
    ...
    NodeIterator.prototype[Symbol.iterator] = function* () { var b; while (b = this.nextNode()) yield b; };

    return this;
  }

  /**
   * tokenize by TinySegmenter and highlight words.
   * highlights are  mainly for Japanese texts because TinySegmenter is Japanese-only tokenizer.
   */
  AutoHighlighter.prototype.highlight_words_tinySegmenter = function () {

    if (document.getElementsByClassName("auto_highlighter").length) {
    ...
    Array.prototype.tallyCount = function () {
      var count = {};
      this.forEach((key) => { count[key] = (count[key] || 0) + 1; });
   ...
   */
  AutoHighlighter.prototype.remove_all_highlights = function () {
    document.querySelectorAll(".auto_highlighter").forEach((e) => {
      if (!e.parentElement) return;

      var prev_text_node = null;
      const child_nodes = [...e.parentElement.childNodes];
  ...
  autohl.highlight_words_tinySegmenter();
